...
void chat(int sockfd, std::string data) 
{ 
    int n = data.length(); 
    ...
    //char buff[MAX1]; 
    write(sockfd, char_array, sizeof(char_array)); 
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        int frame_width = 200;//(int) cv::vcap.get(cv::CV_CAP_PROP_FRAME_WIDTH);
        int frame_height = 200;//(int) cv::vcap.get(cv::CV_CAP_PROP_FRAME_HEIGHT);
